...
import com.compte.compte.config.KafkaProperties;
...
class CompteKafkaResourceIT {
    ...
    void setup() {
        ...
        CompteKafkaResource kafkaResource = new CompteKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/compte-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/compte-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
